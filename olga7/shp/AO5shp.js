...
(function () {              // ---------------------------------------------- shp/AO5shp ---
    ...
    const
        ...
        DblClick = e => {
            if (e.currentTarget !== e.target && e.target.ondblclick) {
                if (debug > 0)
                    console.error("%c%s", C.fmtErr, C.MakeObjName(e.target), ` - тег имеет свой dblclick-обработчик — пропускаем`)
                return
            }
            ...
            if (debug > 0)
                console.log("%c%s", C.fmtOK, `расфиксация '${aO5.cnst.id}' по событию '${e.type}'`)
        },
        IsOnlyTranslate = nst => {
        ...
        Init = aO5 => {
            ...
            if (!t || !(z === "normal" || Number(z) === 1)) {
                ...
                window.olga7.C.ConsoleLog(aO5.name, ` - теги с ` + err + ` НЕ обрабатываются`, 1, 0, add)
    ...
    class AO5 {
        ...
        constructor(shp, quals) {
            shp.aO5shp = this
            this.name = window.olga7.C.MakeObjName(shp)
            this.shdw = shp
        ...
        DoFix(x, xO5) {
            ...
            if (debug) {
                ...
                if (this.act.isfix && !(fixs.T.xO5 || fixs.L.xO5 || fixs.R.xO5 || fixs.B.xO5))
                    console.log("%c%s", C.fmtErr, `DoFix ${this.name} отмечено фиксированным`, ' хотя fixs пусто')

                if (x && xOld === xO5)
                    console.log("%c%s", C.fmtErr,

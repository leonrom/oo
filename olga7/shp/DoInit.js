...
(function () {
    ...
    const
        ...
        Init = () => {
            ...
            if (!found)
                console.log("%c%s", C.fmtErr, `Контейнера с классом 'olga-start' не содержат '${wshp.W.class}'`,
        ...
        ReadCls = (aO5, ss) => {
            ...
            if (errs.length)
                console.error("%c%s", C.fmtErr, `Для ${aO5.name} не опр. квалиф.: ` + errs.join(', '))
        },
        ...
        return {
            ...
            unobserve: (tag) => {
                ...
                if (state.elements.length === 0) {
                    ...
                    state.observer = null
                    if (debug)
                        console.log("%c%s", C.fmtOK, `observe: `, ` отключено полностью`)
                }
            },

(function () {              // ---------------------------------------------- o7 ---
    ...
    class TC {
        ...
        AddModuleSub(modul, submod, funcs) {
             ...
             */
            const
                scrpt = this.#AddScrpt(modul, null),
        ...
        #StartLoad(scrpt, src, text) {
            const timer = setTimeout(() => {
                if (!scrpt.W) {
                    scrpt.err = `Таймаут загрузки модуля`
                    console.error('%c%s', this.fmtErr, `${scrpt.err}`, ` ${text} (src: ${src})`);
                }
        ...
        #StartWait() {
            return setTimeout((scrpt, modul, incl) => {
                if (!scrpt.W) {
                ...
                for (const incl of W.incls) {
                    ...
                    if (this.IsUnDefined(msub.timer)) {
                        ...
                        if (md.iscomp)      // ждем, когда "выстрелит" в этом модуле
                            msub.timer = this.#StartWait(scrpt, modul, incl)
                        else {
                            const script = document.createElement('script')
                            msub.timer = this.#StartLoad(
                                scrpt,
                                W.cls.path + incl + '.js',
                                ` sub='${modul}'`
                            )
            ...
            for (const script of document.scripts) {
                ...
                if (orig && orig[0] === '+') {
                    ...
                    if (md.fn !== ofn) {
                        scrpt.orig = orig
                        if (!scrpt.timer)
                            scrpt.timer = this.#StartLoad(script,
                                this.urlrfs._olga + orig.substring(1),
                                ` модуль '${modul}'`)
    ...
    */
    ...
    (fn => {
        document.readyState === 'loading'
            ...
            : fn();
    })(C.LoadModules);       //  загрузка подключаемых скриптов

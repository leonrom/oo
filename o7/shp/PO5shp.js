...
const
    saved = {
        ...
        Act: (pO5, typ) => {
            ...
            if (
                ...
                if ((dV || dH) && !blks)
                    DoChgs.MakeScroll(dV, dH, pO5, true)
    ...
    IsFinal = tag => {
        return tag.aO7shp ||            // контейнер сам является подвисабельным тегом
...
export class PO5shp {
    ...
    constructor(tag, nst) {
        ...
        Object.assign(this, {
            ...
            scrls: Object.freeze(PO5shp.Scrls(tag, nst)),
        ...
        if (!observer){
            observer = new IntersectionObserver(Observe, {
                ...
                C.cleanup.push(() => observer.disconnect())
        }
        observer.observe(tag)

        if (C.consts.debug > 1)
            console.log(`PO5shp создано ${this.name}`)
    }

    ...
    const
        ...
        C = new class {
            ...
            cls = Object.freeze({
                ...
                path: W.cls.curScript.src.match(/\S*\//)[0] + modul + '/',
            })
            ...
            constructor() {
                ...
                this.debug = !m ? 0 : (m[1] === undefined || m[1] === "") ? 1 : (isNaN(+m[1]) ? 3 : +m[1])
            ...
            AddModuleSub(modul, submod, funcs) {
                 ...
                 */
                const
                    scrpt = this.scrpts[modul] ??= AddScrpt(modul, null),
                ...
                scrpt.msubs ??= {}
                if (scrpt.msubs[submod]) {   // был ранее создан в AddModule
                    if (!this.IsUnDefined(msub.timer))
                        clearTimeout(msub.timer)
                    msub.timer = 0
                }
                else
                    scrpt.msubs[submod] = { timer: 0 }  // таймер не нужен

                msub.done = true    // проверка,- а вдруг уже всё подмодули загружены
            ...
            AddModule(W) {
                const
                    modul = W.cls.modul,
                    scrpt = this.scrpts[modul] ??= AddScrpt(modul, W),
                    wshp = window.olga7[modul] ??= {},
                    md = GetModulName(W.curScript.src)
                ...
                if (W.incls) {
                    ...
                    for (const incl of W.incls) {
                        ...
                        if (this.IsUnDefined(msub.timer)) {
                            msub.done = false
                            if (md.iscomp)      // ждем, когда "выстрелит" в этом модуле
        ...
        LoadModules = function () {
            const ofn = GetModulName(C.cls.curScript)
            for (const script of document.scripts) {
                const orig = script.dataset.src
                if (orig && orig[0] === '+') {
                    const
                        md = GetModulName(orig),
                        modul = md.modul,
                        scrpt = C.scrpts[modul] ??= AddScrpt(modul, null)
                    ...
                    if (md.fn !== ofn) {
                        scrpt.orig = orig
                        if (!scrpt.timer)
                            scrpt.timer = StartLoad(script, script,
                                C.urlrfs._olga + orig.substring(1),
                                ` модуль '${modul}'`)

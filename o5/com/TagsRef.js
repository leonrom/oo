...
ReplaceTag = (tagName, change, adrName, url, errs) => {
		...
		ConvertScripts = () => {
			...
			incls.forEach(incl => { if (incl) _needs[incl] = 1 })
			for (const script of document.scripts) {
				...
				if (incls.length > 0)
					if (_needs[td.modul]) _needs[td.modul] = 0
					else {
						igns.push(td.modul)
						continue
					}
			...
			const errneeds = []
			for (const need in _needs) 
				if (_needs[need]) errneeds.push(need)

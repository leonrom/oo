...
const
    ...
    DebugShowRez = oO5s => {
        ...
        W.ConsoleInfo(`Обработка ${head}`, rez.length, rez)
        ...
        W.ConsoleInfo(`Базы ${head}`, rez.length, rez)
        ...
        W.ConsoleInfo(`pOuts ${head}`, rez.length, rez)
        ...
        W.ConsoleInfo(`Фреймы ${head}`, rez.length, rez)
    },
    ReadCls = (aO5, ss) => {
        const
            errs = [],
        ...
        if (errs.length)
            console.error("%c%s", W.consts.fmtErr, `Для ${aO5.name} не опр. квалиф.: ` + errs.join(', '))
    },
    Observe = entries => {
        ...
        if (newO5s.size > 0) {
            ...
            for (const aO5 of newO5s) {
                if (PBases.AddToBase(aO5))  // если добавилась новая база
                    isNew = true
    ...
    return {
        ...
        unobserve: (tag) => {
            ...
            if (state.elements.length === 0) {
                ...
                state.observer = null
                if (W.consts.debug)
                    console.log("%c%s", W.consts.fmtOK, `observe: `, ` отключено полностью`)
            }
        },
...
export const DoInit = {
    ReadAttrs: aO5 => {
        ...
        wshp.MakeFrames(aO5, sdivs.split(','))
    },
    Init:w => {
        W=w
        let found;
        for (const mtag of W.mtags) {
            if (
        ...
        if (!found)
            console.log("%c%s", W.consts.fmtErr, `Контейнера с классом 'olga-start' не содержат '${W.class}'`,
